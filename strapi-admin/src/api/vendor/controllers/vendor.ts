...
export default factories.createCoreController(
  ...
  ({ strapi }) => ({
    ...
    async findByPincode(ctx) {
      try {
        ...
        const serverUrl =
          ...
          `http://${strapi.config.server.host}:${strapi.config.server.port}`;

        // Find all vendors with populated products and categories
        const vendorsData = await strapi.entityService.findMany(
          "api::vendor.vendor",
          {
            populate: {
              products: {
        ...
        if (!vendorsData || !Array.isArray(vendorsData)) {
          console.error(
            "Invalid vendors data returned from Strapi:",
            vendorsData
          );
          return {
            serviceable: false,
        ...
        const serviceableVendors = vendors.filter((vendor) => {
          try {
            ...
            return (
              ...
              servicePincodes.includes(pincode)
            );
          } catch (error) {
            // If there's any error processing service_pincodes
            console.error(
              "Error checking service_pincodes for vendor:",
              vendor?.id,
              error
            );
            return false;
              ...
              if (product && product.id) {
                ...
                if (product.image) {
                  ...
                  // Case 2: Nested data structure with documentId
                    ...
                    const imageUrl =
                      product.image.url ||
                      `/uploads/${product.image.hash}${product.image.ext}`;
                  ...
                  // Case 3: Already in Strapi format
                    ...
                    if (product.image.data.attributes.url) {
                      product.image.data.attributes.url = ensureAbsoluteUrl(
                  ...
                  // Case 4: Handle array of images (take the first one)
                    ...
                    if (firstImage.url) {
                      ...
                      if (firstImage.data.attributes.url) {
                        firstImage.data.attributes.url = ensureAbsoluteUrl(
                ...
                console.log("Formatted product data:", {
                  ...
                  image: formattedProduct.attributes.image
                    ? formattedProduct.attributes.image.data?.attributes?.url
                    : "No image",
                });
        ...
        const categories = Array.from(categoriesMap.values()).map(
          (category) => {
            ...
            if (category.image) {
              ...
              if (category.image.url) {
                ...
                const imageUrl =
                  category.image.url ||
                  `/uploads/${category.image.hash}${category.image.ext}`;
                const absoluteUrl = ensureAbsoluteUrl(imageUrl);
                formattedCategory.icontype = {
                  data: {
                    attributes: {
            ...
            console.log("Category data:", {
              ...
              image: category.image
                ? ensureAbsoluteUrl(category.image.url) ||
                  (category.image.hash
                    ? ensureAbsoluteUrl(
                        `/uploads/${category.image.hash}${category.image.ext}`
                      )
        ...
        console.log(
          `Found ${categories.length} categories and ${allProducts.length} products for pincode ${pincode}`
        );
        ...
        const response = {
          serviceable: true,
          message:
            serviceableVendors[0]?.delivery_message ||
        ...
        return ctx.badRequest(
          "Error checking pincode serviceability: " + error.message
        );
      }
    },
    ...
    // In your Strapi vendor-auth plugin's controller (e.g., vendor-auth.js or vendor-auth.ts)
    ...
    async login(ctx) {
      ...
      try {
        const vendor = await strapi.db.query("api::vendor.vendor").findOne({
          where: { phone },
          populate: { logo: true }, // Correct populate option
        });
        ...
        // Validate password (using Strapi's user service or a custom function)
        const isValidPassword = await strapi.plugins[
          "users-permissions"
        ].services.user.validatePassword(password, vendor.password); // Replace with your password validation
        ...
        // Return vendor data and JWT
        return ctx.send({ jwt, vendor });
      } catch (error) {
